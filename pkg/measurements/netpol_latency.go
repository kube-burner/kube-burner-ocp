...
func NewNetpolLatencyMeasurementFactory(configSpec config.Spec, measurement types.Measurement, metadata map[string]any, labelSelector string) (measurements.MeasurementFactory, error) {
	return netpolLatencyMeasurementFactory{
		BaseMeasurementFactory: measurements.NewBaseMeasurementFactory(configSpec, measurement, metadata, labelSelector),
	}, nil
}

func (nplmf netpolLatencyMeasurementFactory) NewMeasurement(jobConfig *config.Job, clientSet kubernetes.Interface, restConfig *rest.Config, embedCfg *fileutils.EmbedConfiguration) measurements.Measurement {
...
func (n *netpolLatency) Start(measurementWg *sync.WaitGroup) error {
	...
	connector, err := k8sconnector.NewK8SConnector(n.RestConfig)
	if err != nil {
		log.Error(err)
		return err
	}
