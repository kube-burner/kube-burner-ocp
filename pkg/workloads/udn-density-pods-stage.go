...
// NewUDNDensityPodsStage holds udn-density-pods-stage workload with staged execution
func NewUDNDensityPodsStage(wh *workloads.WorkloadHelper) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Short:        "Runs udn-density-pods workload with staged object creation",
		SilenceUsage: true,
		Run: func(cmd *cobra.Command, args []string) {
			...
			log.Info("Staged execution enabled - objects will be created in stages across all iterations")
			if churnDuration > 0 || churnCycles > 0 {
				log.Info("Churn is enabled and will run after all stages complete")
			}
	...
	cmd.Flags().IntVar(&churnCycles, "churn-cycles", 0, "Churn cycles to execute after staged creation")
