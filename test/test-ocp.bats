...
@test "workers-scale: local-indexing=true" {
  run_cmd kube-burner-ocp workers-scale --uuid=xyz --additional-worker-nodes 1 --metrics-profile custom-metrics.yml
...
@test "workers-scale: enable-autoscaler=true; es-indexing=true" {
  run_cmd kube-burner-ocp workers-scale --uuid="${UUID}" --additional-worker-nodes 1 --metrics-profile custom-metrics.yml --enable-autoscaler=true --es-server=${ES_SERVER} --es-index=${ES_INDEX}
